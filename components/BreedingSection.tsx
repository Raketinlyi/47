import { AlertCircle, Clock, Heart, HeartCrack, Plus, Star, X, XCircle, Zap } from 'lucide-react';
'use client';
...
const BreedableNFT = ({
  ...
  // РџСЂРѕСЃС‚Р°СЏ РЅР°СЃС‚СЂРѕР№РєР° Р°РЅРёРјР°С†РёР№ РґР»СЏ СЃР»Р°Р±С‹С…/СЃСЂРµРґРЅРёС… РџРљ
        ...
        <CardContent className='p-4'>
          <div className='relative'>
            <IpfsImage
              ...
              tokenId={tokenIdDecimal} // РџРµСЂРµРґР°РµРј tokenId РґР»СЏ Р»РѕРєР°Р»СЊРЅРѕРіРѕ РёР·РѕР±СЂР°Р¶РµРЅРёСЏ
            />
...
export const BreedingSection = () => {
  ...
  const handleBreed = async () => {
    ...
    try {
      ...
      // Wait for approval, then immediately breed (no artificial delay)
      try {
        await breedNFTs(
      ...
      toast({
        ...
        description:
          (error as Error).message || 'Failed to process transaction',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <div className='space-y-6'>
        ...
        <div className='flex justify-center gap-6 text-sm'>
          ...
          <div className='text-center'>
            <div className='text-green-300 font-bold'>
              {formatSmart(octaaBalance || '0', 8)} CRAA
            </div>
            <div className='text-slate-400'>
              {t('sections.breed.yourCraaBalance', 'Your CRAA Balance')}
            </div>
          </div>
          <div className='text-center'>
                    ...
                    <div key={parentId} className='flex items-center gap-2'>
                      <div className='text-center'>
                        ...
                        <div className='text-xs text-pink-300 mt-1'>
                          #{parentId} ({gameInfo?.currentStars}<Star className="w-4 h-4 inline mx-1" />)
                        </div>
                      </div>
      ...
      {breedableNfts.length > 0 ? (
        ...
        >
          <HeartCrack className='w-16 h-16 mb-4 text-pink-500 mx-auto' />
      ...
      {isTxSuccess && (
        ...
        >
          <div className='text-pink-400 text-lg flex items-center justify-center gap-2'>
            <Heart className='w-5 h-5 text-pink-400 fill-pink-400' /> Breeding
            Successful!
          </div>
      ...
      {(writeError || txError || isTxError) && (
        ...
        >
          <div className='text-red-400 text-lg flex items-center justify-center gap-2'>
            <XCircle className='w-5 h-5' /> Breeding Failed
          </div>
          <p className='text-sm text-slate-400 mt-1'>
