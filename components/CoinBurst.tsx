...
export default function CoinBurst({
  ...
  // Reduce coin count in lite mode
  const coinCount = isLiteMode ? Math.floor(total / 2) : total;
        ...
        {active && (
          <span className='absolute inset-0 pointer-events-none z-30'>
            {Array.from({ length: coinCount }).map((_, i) => {
              const angle = (360 / coinCount) * i + Math.random() * 15;
...
export function HeartBurst({
  ...
  // Reduce heart count in lite mode
  const heartCount = isLiteMode ? Math.floor(total / 2) : total;
      ...
      <AnimatePresence>
        {active &&
          Array.from({ length: heartCount }).map((_, i) => {
            const angle = (360 / heartCount) * i + Math.random() * 15;
            const dist = 110 + Math.random() * 40;
            const x = dist * Math.cos((angle * Math.PI) / 180);
            const y = dist * Math.sin((angle * Math.PI) / 180);
            return (
