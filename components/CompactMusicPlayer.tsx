...
const validateAudioUrl = (url: string): boolean => {
  try {
    ...
    return TRUSTED_AUDIO_DOMAINS.some(
      domain =>
...
const sanitizeTrackData = (track: unknown) => {
  ...
  return {
    ...
    url: validateAudioUrl(String(trackObj.url || ''))
      ? String(trackObj.url)
      : '',
...
export const CompactMusicPlayer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleEnded = () => {
      ...
      if (nextTrack) {
        ...
        // small delay to ensure src is set, then play next track
        setTimeout(() => {
          const a = audioRef.current || getGlobalAudioElement();
          a?.play().catch(() => {
            /* ignore play errors */
          });
        }, 120);
  ...
  const selectTrack = async (
    track: { id: string; name: string; url: string; theme: string } | null
  ) => {
    if (!track) return;
  ...
  return (
    <div className='flex items-center gap-2'>
      {/* Global <audio> element СЃРѕР·РґР°С‘С‚СЃСЏ РІ ClientLayout (lib/globalAudio) */}
