  ...
  return (
    <div className={`pointer-events-none absolute inset-0 z-0 ${className}`}>
      ...
      <motion.div
        ...
        style={{
          background: `linear-gradient(90deg, transparent 0%, ${colors.sweep} 40%, ${colors.sweep} 60%, transparent 100%)`,
          filter: isLiteMode ? 'none' : 'blur(8px)',
          rotate: '10deg',
        }}
        initial={{ x: '-60%' }}
        animate={{ x: ['-60%', '160%'] }}
      ...
      {Array.from({ length: isLiteMode ? 1 : 3 }).map((_, i) => (
        <motion.div
          key={`arc-${i}`}
          ...
          style={{
            ...
            filter: isLiteMode ? 'none' : 'drop-shadow(0 0 6px currentColor)',
            color: colors.arc,
          }}
...
function getColors(tint: Tint, k: number) {
  ...
  const alpha = (a: number) =>
    `hsla(${h}, ${s}%, ${Math.min(100, Math.max(0, l))}%, ${a})`;
  return {
    glowA: alpha(0.2 * k),
    glowB: alpha(0.3 * k),
    ringA: alpha(0.55 * k),
