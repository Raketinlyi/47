...
interface AshRainProps {
  /** Amount of ash particles per 100 vw (в‰€ screen width). Default = 18 */
  density?: number;
...
export default function AshRain({
  ...
  const ashes = useMemo(() => {
    ...
    return Array.from({ length: count }, () => ({
      ...
      opacity: isMobile ? 0.3 + Math.random() * 0.3 : 0.4 + Math.random() * 0.4, // slightly higher opacity for better visibility
