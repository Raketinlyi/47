...
export default function CubeObservers({
  ...
  // Get phrases from translations вЂ“ memoized so reference is stable between renders
  ...
  // on success вЂ“ update phrases & after few seconds clear
  useEffect(() => {
    if (
      phase !== 'success' ||
      observers.length === 0 ||
          ...
          return (
            ...
            >
              ...
              <img
                src={c.img}
                alt='cube observer'
                className={`${isMobile ? 'w-14 h-14' : 'w-48 h-48'} drop-shadow-xl`}
              />
              ...
              <motion.div
                ...
                className={`absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-white/90 text-gray-900 rounded ${isMobile ? 'px-2 py-1 text-xs max-w-[140px]' : 'px-3 py-2 text-sm max-w-[220px]'} font-semibold shadow whitespace-normal break-words text-center`}
              >
                {c.phrase}
