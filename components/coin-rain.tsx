...
export function CoinRain() {
  ...
  useEffect(() => {
    ...
    const coinCount = isLiteMode ? Math.floor(COINS * 0.4) : COINS; // Reduce by 60% in lite mode
    const arr = Array.from({ length: coinCount }).map(() => ({
      delay: (Math.random() * 5) / speedMultiplier,
      duration: (6 + Math.random() * 4) / speedMultiplier,
      size: 12 + Math.random() * 16,
      x: -50 + Math.random() * 100,
    }));
    setVariants(arr);
  }, [isLiteMode]); // React to lite mode changes
