...
import { ChevronDown, Music, PartyPopper, Sparkles, Star, VolumeX, Zap } from 'lucide-react';
...
const SpeechBubble = React.memo(function SpeechBubble({
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {/*
        вЂў max-w-[80vw] РІР‚вЂќ on mobile devices, the bubble expands to a maximum of 80% of the screen width,
          so long phrases do not go beyond the window.
        вЂў sm:max-w-[220px] РІР‚вЂќ on desktops, the limit remains the same.
        вЂў break-words РІР‚вЂќ break long words.
      */}
...
export function CubeAnimation({ desktopScale = 1 }: CubeAnimationProps) {
  ...
  // Ensure we reuse the global audio element created in ClientLayout
  useEffect(() => {
    try {
      if (!audioRef.current) {
        const ga = getGlobalAudioElement();
        if (ga) {
          (
            audioRef as unknown as React.MutableRefObject<HTMLAudioElement | null>
          ).current = ga;
        }
      }
    } catch {
  ...
  const cubeSize = isMobile
    ? 120
    : Math.round(450 * Math.max(0.5, desktopScale));
  ...
  return (
    ...
    >
      {/* Use the global audio element (created in ClientLayout) so music is shared across components */}
      {/* We intentionally do not render a local <audio> here; cube-animation will reuse the global element. */}
