    ...
    const fetchBurnedNfts = async () => {
      ...
      try {
        ...
        try {
          ...
          /* ignore вЂ“ fallback to local time */
        }
        ...
        // Cache for split-parameters (by waitPeriod) вЂ“ to avoid hitting contract multiple times
        ...
        for (let i = 0; i < uniqueIds.length; i += CHUNK) {
          ...
          if (supportsMulticall) {
            ...
            const contracts = slice.map(tokenId => ({
              ...
              args: [BigInt(tokenId), address] as const,
            }));
            ...
            try {
              ...
              const chunkResults = mcResults
                .map((result, index) => {
                  ...
                  return {
                    ...
                    split,
                    playerShare,
                    isReadyToClaim: !claimed && claimAt <= BigInt(chainNow),
                  };
                })
                .filter(Boolean);
              ...
              if (missingSplits.length > 0) {
                ...
                splitResults.forEach((result, index) => {
                  if (result.status === 'success' && result.result) {
                    const [playerBps, poolBps, burnBps] = result.result as [
                      number,
                      number,
                      number,
                    ];
                    splitCache.set(uniqueWaitPeriods[index], {
                      playerBps,
                      poolBps,
                      burnBps,
                    });
                  ...
                  if (result && 'needsSplit' in result) {
                    ...
                    if (split && result.tokenId) {
                      ...
                      if (mcResult && mcResult.result) {
                        ...
                        if (record && record[1]) {
                          const playerShare =
                            (record[1] * BigInt(split.playerBps)) / 10000n;
                          ...
                          const finalResult: BurnedNftInfo = {
                            ...
                            playerShare,
                            isReadyToClaim:
                              !record[4] && record[2] <= BigInt(chainNow),
                          };
        ...
        localStorage.setItem(
          cacheKey,
          JSON.stringify({ data: serializableData, timestamp: Date.now() })
        );
      } catch (e) {
...
export const useClaimReward = (onSuccess?: () => void) => {
  ...
  // V4: claim now requires both tokenId and nonce
  const claim = async (tokenId: string, nonce: string = '1') => {
    ...
    try {
      ...
      await writeContractAsync({
        ...
        args: [BigInt(tokenId), BigInt(nonce)], // V4: tokenId + nonce
      });
    } catch (e: unknown) {
      const errorMessage =
