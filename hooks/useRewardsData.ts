...
export interface RewardInfo {
  ...
  pairDeposited: string | undefined; // WMON amount user will receive
}

export function useRewardsData() {
  const { nfts, loading } = useUserNFTs();
  const { getBurnRecord, claimBurnRewards, getLPInfo } = useCrazyOctagonGame();
    ...
    try {
      ...
      await Promise.all(
        nfts.map(async (nft: any) => {
          ...
          if (rec) {
            // Also fetch LP info to get WMON amount
            let pairDeposited: string | undefined;
            try {
              const lpInfo = await getLPInfo(String(nft.tokenId));
              if (lpInfo) {
                pairDeposited = lpInfo.pairDeposited;
              }
            } catch {
              // Ignore LP fetch errors
            }
            data.push({
              tokenId: nft.tokenId,
