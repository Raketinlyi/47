// DDoS Protection icons removed for production
...
class DDoSProtection {
  ...
  // CLEANUP_INTERVAL used by setInterval at bottom
  ...
  // Block an IP address
  private static blockIP(ip: string, _reason: string): void {
    const stats = this.ipStats.get(ip) || {
      requests: 0,
    ...
    // РЈРґР°Р»СЏРµРј console.error РґР»СЏ production Р±РµР·РѕРїР°СЃРЅРѕСЃС‚Рёq
    // console.error(`<Ban className="w-4 h-4 inline mx-1" /> DDoS Protection: Blocked IP ${ip} - ${reason} for ${this.BLOCK_DURATION / 1000 / 60} minutes`);
  }
  ...
  static unblockIP(ip: string): boolean {
    ...
    if (stats && stats.blocked) {
      ...
      stats.blockUntil = 0;
      this.ipStats.set(ip, stats);
      // РЈРґР°Р»СЏРµРј console.log РґР»СЏ production Р±РµР·РѕРїР°СЃРЅРѕСЃС‚Рё
      // console.log(`<CheckCircle2 className="w-4 h-4 inline mx-1" /> DDoS Protection: Unblocked IP ${ip}`);
      return true;
    }
    return false;
  }
...
export function securityMiddleware(req: any, res: any, next: any) {
  ...
  res.setHeader(
    ...
    EnhancedRateLimiter.getRemainingRequests(ip)
  );
