// Security logging icons commented out for production
...
export const securityLogger = {
  ...
  logRateLimitViolation: (
    ...
    if (process.env.NODE_ENV === 'development') {
      // Удаляем console.warn для production безопасности
      // console.warn(
      //   `<Clock className="w-4 h-4 inline mx-1" /> RATE_LIMIT: ${ip} exceeded ${limit} requests per ${window}`
      // );
    }
  },
  ...
  logCSRFFailure: (
    ...
    if (process.env.NODE_ENV === 'development') {
      // Удаляем console.warn для production безопасности
      // console.warn(`<Shield className="w-4 h-4 inline mx-1" /> CSRF_FAILURE: ${ip} - Invalid CSRF token`);
    }
  },
  ...
  blockIP: (ip: string, reason: string, durationMinutes: number = 60) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      // Удаляем console.error для production безопасности
      // console.error(
      //   `<Ban className="w-4 h-4 inline mx-1" /> IP_BLOCKED: ${ip} - ${reason} for ${durationMinutes} minutes`
      // );
    }
  },
